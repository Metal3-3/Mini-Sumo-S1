...
analogRead(Redge)< 500 || analogRead(Ledge)< 500 ) { digitalWrite(ArduLed, HIGH);}
...
//for (int i = 0; i < 5; i++){ Set_Motor(0,0,1); digitalWrite(ArduLed, HIGH); tone(Buzzer, 523, 300);
delay(500); noTone(Buzzer); digitalWrite(ArduLed, LOW); delay(200); }
...
if (analogRead(Ledge)<100 && analogRead(Redge)> 100) {
 ...
 Set_Motor(-100, 100, EdgeTurn); // Left Backward, Right Forward, Turning Time Based on ETRN
Trimpot
 LastValue=5;
}
 ...
 Set_Motor(100, -100, EdgeTurn); // Right Backward, Left Forward, Turning Time Based on ETRN
Trimpot
 LastValue=5;
 }
 else if (analogRead(Ledge)< 100 && analogRead(Redge)< 100) {
...
//while (digitalRead(Button)==LOW) {Set_Motor(0, 0, 20); digitalWrite(Buzzer, LOW); LastValue=3;}
digitalWrite(Buzzer, LOW);
